 ...
 * "match shadow color to the background" rule: a cool indigo-tinted shadow reads as depth, a
 * flat gray one reads as grime.
 */
@Composable
fun Modifier.docketCardShadow(
    ...
    spotAlpha: Float = 0.18f
): Modifier = this.shadow(
    elevation = elevation,
    shape = shape,
    ambientColor = tint.copy(alpha = ambientAlpha),
    spotColor = tint.copy(alpha = spotAlpha)
)
